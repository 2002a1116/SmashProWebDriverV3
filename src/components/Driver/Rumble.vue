    ...
    <n-flex vertical>
        ...
        <n-flex justify="center">
            ...
            <n-card :title="$t('text.rumble')+$t('word_sep')+$t('text.settings')">
                ...
                -->
                    <func-switch v-model:value="rumble_enabled" :text="$t('text.rumble')"/>
                    <span>{{ $t('hint.rise_low') }}</span>
                    <func-switch v-model:value="low_amp_rise_hack" :text="$t('text.rise_low')"/>
                    <span>{{ $t('hint.drop_high') }}</span>
                    <func-switch v-model:value="high_amp_drop_hack" :text="$t('text.drop_high')"/>
                </n-flex>
            </n-card>
...
export default {
    ...
    computed:{
        rumble_enabled:{
            get():boolean{
                return !(conf.config_bitmap0&0x20);
            },
            set(v:boolean){
                if(v)
                    conf.config_bitmap0&=(~0x20);
                else
                    conf.config_bitmap0|=0x20;
            }
        },
        ...
        rumble_pattern:{
            get():number{
                return (conf.config_bitmap0>>6)&0x1;
            },
            set(v:number){
                conf.config_bitmap0=(conf.config_bitmap0&0xbf)|((v<<6)&0x40);
            }
        },
        rumble_mode:{
            get():number{
                return (conf.config_bitmap1>>5)&0x1;
            },
            set(v:number){
                conf.config_bitmap1=(conf.config_bitmap1&0xdf)|((v<<5)&0x20);
            }
        },
        low_amp_rise_hack:{
            get():boolean{
                return ((conf.config_bitmap1>>4)&0x1)==1;
            },
            set(v:boolean){
                conf.config_bitmap1=(conf.config_bitmap1&0xef)|(((v?1:0)<<4)&0x10);
            }
        },
        high_amp_drop_hack:{
            get():boolean{
                return ((conf.config_bitmap1>>3)&0x1)==1;
            },
            set(v:boolean){
                conf.config_bitmap1=(conf.config_bitmap1&0xf7)|(((v?1:0)<<3)&0x08);
            }
        },
    },
    methods: {
