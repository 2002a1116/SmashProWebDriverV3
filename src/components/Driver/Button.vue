<template>
    <n-flex vertical>
        ...
        <n-flex vertical>
            ...
            <n-flex justify="center">
                <n-card>
                    <func-switch v-model:value="a_b_swap" :text="$t('text.swap a&b')"/>
                    <func-switch v-model:value="x_y_swap" :text="$t('text.swap x&y')"/>
                    <func-switch v-model:value="dpad_mapping_joystick" :text="$t('text.dpad_map_js')"/>
                </n-card>
            </n-flex>
        </n-flex>
    </n-flex>
...
import { button_sts, conf, hex_to_rgb, rgb_to_hex, send_conf } from '../webusb';
...
export default {
    ...
    computed:{
        dpad_mapping_joystick:{
            get():boolean{
                return ((conf.config_bitmap0>>2)&0x1)>0;
            },
            set(v:boolean){
                if(v)
                    conf.config_bitmap0|=0x4;
                else
                    conf.config_bitmap0&=(~0x4);
            }
        },
        a_b_swap: {
            get(): boolean {
                return (conf.config_bitmap0 & 0x10) != 0;
            },
            set(v: boolean) {
                if (v) conf.config_bitmap0 |= 0x10;
                else conf.config_bitmap0 &= (~0x10);
            }
        },
        x_y_swap: {
            get(): boolean {
                return (conf.config_bitmap0 & 0x08) != 0;
            },
            set(v: boolean) {
                if (v) conf.config_bitmap0 |= 0x08;
                else conf.config_bitmap0 &= (~0x08);
            }
        },
        key_color_bg: {
            get() {
